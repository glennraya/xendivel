...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased relative h-screen grid bg-gray-300">
        ...
        <div class="container mt-8 mx-auto flex flex-col items-center gap-4">
            ...
            <div class="mt-8 flex w-[500px] flex-col rounded-md border border-gray-300 bg-white">
                ...
                <div class="px-8">
                    ...
                    <div id="card-mode-description" class="mb-4 rounded-md border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                ...
                >
                    ...
                    <div id="authorized-charge-actions" class="mt-6 hidden flex-col gap-4 rounded-md border border-gray-300 bg-gray-50 p-4 text-sm">
                        <div class="flex flex-col gap-1">
                            <span class="font-bold">Authorized Charge Actions</span>
                            <span>Charge ID: <span id="authorized-charge-id">N/A</span></span>
                            <span>Status: <span id="authorized-charge-status">N/A</span></span>
                            <span>Authorized Amount: <span id="authorized-charge-amount">N/A</span></span>
                            <span>External ID: <span id="authorized-charge-external-id">N/A</span></span>
                        </div>
                        <input id="capture-amount" type="text" class="rounded-md border border-gray-300" placeholder="Capture amount" />
                        <input id="reversal-external-id" type="text" class="rounded-md border border-gray-300" placeholder="Reversal external ID (optional when auto ID is enabled)" />
                        <div id="authorized-charge-action-buttons" class="grid grid-cols-2 gap-4">
                            <button type="button" id="capture-authorized-charge-btn" class="rounded-md bg-black py-3 text-sm font-bold uppercase text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-black">
                                Capture Authorized Charge
                            </button>
                            <button type="button" id="void-card-authorization-btn" class="rounded-md bg-red-600 py-3 text-sm font-bold uppercase text-white hover:bg-red-500 disabled:cursor-not-allowed disabled:opacity-50">
                                Void Authorization
                            </button>
                        </div>
                    </div>
                ...
                >
                    <button
                        ...
                        class="col-span-6 text-sm uppercase rounded-md bg-black py-3 font-bold text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-black"
                    >
                        Charge with eWallet
                    </button>
                </div>
        ...
        {{-- Process for tokenizing the card details, validation,
             authorization, capture, and auth reversal. --}}
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                var amountInput = document.getElementById('amount-to-pay')
                var cardPaymentMode = document.getElementById('card-payment-mode')
                var cardModeDescription = document.getElementById('card-mode-description')
                ...
                var authorizedChargeActions = document.getElementById('authorized-charge-actions')
                var authorizedChargeActionButtons = document.getElementById('authorized-charge-action-buttons')
                var captureAuthorizedChargeBtn = document.getElementById('capture-authorized-charge-btn')
                var voidCardAuthorizationBtn = document.getElementById('void-card-authorization-btn')
                var captureAmountInput = document.getElementById('capture-amount')
                var reversalExternalIdInput = document.getElementById('reversal-external-id')
                ...
                var apiResponsePre = document.getElementById('api-response')
                ...
                cardPaymentMode.addEventListener('change', function () {
                    cardModeDescription.textContent = this.value === 'authorize'
                ...
                chargeCardBtn.addEventListener('click', function (event) {
                    ...
                    var cardNumber = Xendit.card.validateCardNumber(form.querySelector('#card-number').value)
                    var expiryDate = Xendit.card.validateExpiry(
                    ...
                    var amountToPay = amountInput.value

                    if (form.querySelector('#card-cvn').value === '') {
                        showError('Card CVV/CVN is optional when creating card token, but highly recommended to include it.')
                        chargeCardBtn.disabled = false
                        return
                    }

                    if (amountToPay < 20 || cardNumber === false || expiryDate === false || cvn === false) {
                        showError('Invalid card details or amount. The amount must be at least 20.')
                        chargeCardBtn.disabled = false
                        return
                    }
                    ...
                    Xendit.card.createToken({
                        ...
                        is_multiple_use: save_card === true,
                ...
                captureAuthorizedChargeBtn.addEventListener('click', function () {
                    if (!authorizedChargeState || !authorizedChargeState.id) {
                        return
                    }
                    ...
                    axios.post('/capture-card-charge', {
                        charge_id: authorizedChargeState.id,
                        amount: parseInt(captureAmountInput.value, 10),
                    })
                    .then(function (response) {
                        handleCardResponse(response.data)
                        hideAuthorizedChargeActionButtons()
                    })
                ...
                voidCardAuthorizationBtn.addEventListener('click', function () {
                    if (!authorizedChargeState || !authorizedChargeState.id) {
                        return
                    }
                    ...
                    if (reversalExternalIdInput.value.trim() !== '') {
                        payload.external_id = reversalExternalIdInput.value.trim()
                    }

                    axios.post('/void-card-authorization', payload)
                    .then(function (response) {
                        handleCardResponse(response.data)
                    })
                    .catch(showAxiosError)
                ...
                function authenticationHandler(err, response) {
                    ...
                    switch (response.status) {
                        case 'VERIFIED':
                            submitCardPayment(response.id, response.credit_card_token_id)
                            break
                        ...
                        case 'FAILED':
                            setIframeSource('payer-auth-url', '')
                            authDialog.style.display = 'none'
                            apiResponsePre.textContent = JSON.stringify(response, null, 2)
                ...
                function submitCardPayment(authId, cardToken) {
                    var endpoint = cardPaymentMode.value === 'authorize'
                        ? '/authorize-card'
                        : '/pay-with-card'
                ...
                function handleCardResponse(data) {
                    ...
                    if (save_card === true) {
                        ...
                        multiUseToken.style.display = 'none'
                    }

                    if (data.status === 'AUTHORIZED' || authorizedChargeState !== null) {
                        setAuthorizedChargeState(data)
                    }
                }

                function setAuthorizedChargeState(data) {
                    authorizedChargeState = {
                        id: data.id,
                        status: data.status,
                        authorized_amount: data.authorized_amount || data.capture_amount || '',
                        external_id: data.external_id || '',
                    }

                    document.getElementById('authorized-charge-id').textContent = authorizedChargeState.id || 'N/A'
                    document.getElementById('authorized-charge-status').textContent = authorizedChargeState.status || 'N/A'
                    document.getElementById('authorized-charge-amount').textContent = authorizedChargeState.authorized_amount || 'N/A'
                    document.getElementById('authorized-charge-external-id').textContent = authorizedChargeState.external_id || 'N/A'

                    if (authorizedChargeState.authorized_amount) {
                        captureAmountInput.value = authorizedChargeState.authorized_amount
                    }

                    if (authorizedChargeState.external_id) {
                        reversalExternalIdInput.value = authorizedChargeState.external_id
                    }

                    authorizedChargeActions.style.display = 'flex'

                    if (authorizedChargeState.status === 'AUTHORIZED') {
                        showAuthorizedChargeActionButtons()
                    }
                }
                ...
                function showAxiosError(error) {
                    var err = parseAxiosError(error)
                    showError(err.message)
                }

                function parseAxiosError(error) {
                    if (error && error.response && typeof error.response.data.message === 'string') {
                        try {
                    ...
                    return {
                        message: error && error.message ? error.message : 'Unable to process the request.',
                    }
                }
                ...
                function setActionButtonsDisabled(disabled) {
                    captureAuthorizedChargeBtn.disabled = disabled
                    voidCardAuthorizationBtn.disabled = disabled
                }

                function showAuthorizedChargeActionButtons() {
                    authorizedChargeActionButtons.style.display = 'grid'
                }

                function hideAuthorizedChargeActionButtons() {
                    authorizedChargeActionButtons.style.display = 'none'
                }
